...
- **Common mistakes:** Skipping the capability filter when resolving candidates. `manual` routing with no explicit placement must fail loudly, not silently fall back to `waterfall`. Building `build_fast_path_tables` from `SchedulePlan` instead of `CompiledProject` — the runtime dispatch path (dynamic coordinator *and* the fast-path table) only ever chooses between a stage's `primary_core`/`fallback_core`, while the scheduler's `_select_core` can legitimately pick a third `prefer_balance` candidate for offline bookkeeping only (see `test_advanced_scheduler.py`'s `ScheduledCoreStaysWithinRuntimeDispatchChoicesTests`).
...
- **Key functions and subparts (one renderer per emitted file):**
  ...
  - `_render_coordinator` (+ `_stage_case_entries`, `_flow_last_stage_entries`, `_flow_slot_entries`) — the multi-issue flow orchestrator and its `MAX_IN_FLIGHT` localparam. Its result matcher is deliberately relaxed to recognize a fast-path chain's eventual hub-bound packet (see the Coordinator depth contract's scoped supersession, above) — a 4-line diff, otherwise untouched.
  - `_render_top` — the grid top level and the `obs_total_*` aggregation; also computes each topology's `COORD_DST_SENTINEL` and owns the widened data-plane payload layout (`DATA_IS_FAST_PATH_LSB`/`DATA_NEXT_*_LSB`/`DATA_B_REG_LSB`, appended after the original, unmoved result fields so `coord_result_*` needs no awareness of the widening).
