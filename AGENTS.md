...
**Vocabulary (defined on first use):** **WAU** = Waterfall Arithmetic Unit. **Flow** = a DAG (or linear stage chain) of arithmetic nodes with one entry and one exit, identified by a `flow_id`. **Program** = a scheduling container grouping flows with priority/replica/load-balance policy. **Core** = one ALU + station at grid coordinate `(x, y, z)`. **Station** = the per-core dispatch/latency/cache wrapper. **Highway** = a router run carrying packets between cores; by default there is one per *line* of cores, each with its own coordinator hub. **Coordinator** = the flow orchestrator that dispatches stages and collects results. **`.cw`** = the repository's kernel language. **MMIO** = the memory-mapped host register file (`wau_host_mmio`).
...
- **Program concurrency is a window, never truncation.** `program.replicas * len(program.flows)` is the number of flow instances the offline schedule MUST contain. `program.max_parallel_flows` limits how many instances may overlap by adding ordering-only dependencies between successive waves; those edges MUST NOT appear in `data_dependency_keys` or transfer-hop metrics. Owned by `_build_runtime_nodes` in [`scheduler.py`](src/python/waugen/scheduler.py); guarded by `test_max_parallel_flows_caps_waves_without_dropping_replicas` in [`tests/python/test_program_stress.py`](tests/python/test_program_stress.py).
- **Schedule/runtime core agreement.** The offline schedule and schedule-derived contract ROM MUST select only a compiled stage's `primary_core` or `fallback_core`, because those are the only choices emitted into the coordinator/fast-path runtime. Extra `candidate_cores` remain compiler analysis inputs; predicting one as an executed core is prohibited. Guarded for the tracked multiprogram fixture by [`tests/python/test_advanced_scheduler.py`](tests/python/test_advanced_scheduler.py).
- **Coordinator depth and transaction-tag contract.** `coordinator.max_in_flight` MUST be in `[1,16]` and MUST match the emitted `WAU_COORD_MAX_IN_FLIGHT` macro and the `wau_coordinator` `MAX_IN_FLIGHT` localparam. Each accepted input owns one slot; that slot index is carried as an 8-bit internal transaction tag through the control payload, `wau_core_station` (including direct fast-path hops), the data payload, and the result matcher. Therefore repeated inputs with the **same flow id** may be resident concurrently and MUST resolve to their own accumulator contexts. Completion may occur out of order internally, but `slot_order`/`alloc_order`/`retire_order` MUST present outputs to the unchanged host interface in original acceptance order, so no public tag is required. A single in-flight flow stays **cycle-identical** to the legacy serial design (`tb_wau_top_demo` timing, CW exec per-case latencies, and the scoreboard must not regress); unknown flow ids stay accepted-but-dropped. The result matcher additionally accepts `result_pkt_stage_id >= slot_stage[ri]` so a chain of per-core fast-path hops can run ahead of the coordinator's stage counter. Owned by `_render_coordinator`, `_render_core_station`, `_render_core`, `_render_top`, and `_render_fabric_binding` in [`verilog_emit.py`](src/python/waugen/verilog_emit.py), plus the mirrored `TB_DATA_*_LSB` layout in the viewer; proven by [`tests/rtl/tb_wau_coordinator_multiissue.v`](tests/rtl/tb_wau_coordinator_multiissue.v) (two same-flow transactions admitted before either retires, correct tagged results in acceptance order, ≥2 busy cores), [`tests/python/test_coordinator_config.py`](tests/python/test_coordinator_config.py), and the 4x4 viewer test in [`tests/python/test_viewer_data_trace.py`](tests/python/test_viewer_data_trace.py).
- **Per-core fast-path table contract.** `compiler.station_program.enabled` (default `false`) and `.table_bits` (default `5`, range `[1,8]`) gate and size a per-core static dispatch table: when enabled, a core that finishes a **non-final** flow stage looks up `(flow_id, stage_index)` in its own table and, on a hit, hands its result **directly to the next stage's core** via the data plane's local port (`wau_core`'s `self_dispatch_*`/`peer_busy` ports), bypassing `wau_coordinator` for that hop entirely — the mechanism that turns interior stage transitions from a coordinator round-trip into a direct core-to-core handoff. The current transaction tag MUST be forwarded unchanged across every such hop. Tables are built **exclusively from `CompiledProject`** (`compiler.build_fast_path_tables`), never from `SchedulePlan`; both runtime paths and the schedule are restricted to the stage's `primary_core`/`fallback_core`. A flow's **last** stage never gets an entry, which structurally guarantees every flow chain still reaches the coordinator's hub eventually. Capacity is exactly `2**table_bits` entries per core (no reserved sentinel: the RTL lookup is a `case ((flow_id, stage_id))`-keyed function per `_core_fast_path_lookup_function`, not a literal indexed ROM); a `(flow_id, stage_index)` pair that does not fit is recorded in `overflowed_keys` and silently keeps using the legacy coordinator path — **no correctness risk, just no speedup**, identical in spirit to `enable_auto_adapt`'s pre-existing fallback story. The same "no speedup, still correct" outcome also covers a fast-path hop whose destination is on a **different highway line** under the default `lines` topology: there is no hub-to-hub bridge (only hub-to-coordinator), so such a packet is safely absorbed by the coordinator's tagged result matching and the next stage is re-dispatched normally — `build_fast_path_tables` does not currently avoid allocating table capacity to these, which is a known, harmless inefficiency (see [ROADMAP.md](ROADMAP.md)). Under `chain`/`matrix`, `excluded_destinations` (`_fast_path_excluded_destinations` in `verilog_emit.py`) unconditionally excludes the one core index (`COORDINATOR_CORE_INDEX`, always `0`) the coordinator shares a physical local port with, since a fast-path packet routed there would be misread as a legacy result rather than reaching that core's own station. `device.enable_runtime_auto_adapt` (default now `false`, "disable in-circuit schedulers unless strictly needed") drives `wau_coordinator`'s `enable_auto_adapt` **reset value** (`WAU_DEFAULT_AUTO_ADAPT`) via `_render_host_mmio`, still live-toggleable over MMIO CTRL bit 1 exactly as before. Owned by `StationProgramSpec` in [`config.py`](src/python/waugen/config.py); `build_fast_path_tables`/`FastPathHop`/`CoreFastPathTable` in [`compiler.py`](src/python/waugen/compiler.py); `_core_fast_path_lookup_function`, `_fast_path_word_layout`, `_fast_path_excluded_destinations`, and the `wau_core_station`/`wau_core`/`wau_top`/`_render_fabric_binding` changes in [`verilog_emit.py`](src/python/waugen/verilog_emit.py). Proven by [`tests/rtl/tb_wau_core_fast_path_overlap.v`](tests/rtl/tb_wau_core_fast_path_overlap.v) (genuine speedup + cross-line safety), [`tests/rtl/tb_wau_station_program_degenerate.v`](tests/rtl/tb_wau_station_program_degenerate.v) (disabled = byte-identical legacy timing), [`tests/rtl/tb_wau_station_program_overflow.v`](tests/rtl/tb_wau_station_program_overflow.v) (capacity overflow stays correct), and [`tests/python/test_station_program_table.py`](tests/python/test_station_program_table.py) / [`tests/python/test_station_program_config.py`](tests/python/test_station_program_config.py).

- **Highway topology contract.** `device.highway.topology` MUST be `lines` (default), `chain` or `matrix`, and MUST reach both `WAU_HIGHWAY_TOPOLOGY_{LINES,CHAIN,MATRIX}` / `WAU_HIGHWAY_PORT_COUNT` / `WAU_HIGHWAY_LINE_{COUNT,SIZE}` *and* the emitted router's `PORT_COUNT` localparam and port set. `lines` emits **one independent highway per line of cores** (`grid.y * grid.z` of them, `LOCAL`/`PREV`/`NEXT` only), each reaching the coordinator through **its own hub** on the line's west end; `chain` emits one index-order highway per layer (`+UP`/`DOWN`); `matrix` keeps the seven-port X-then-Y-then-Z mesh. Only `matrix` may contain `% GRID_X` / `/ GRID_X` — the index/bounds compare is what removes the per-port `LPM_DIVIDE` from the other two. **Lines MUST stay independent**: no wire, no back-pressure and no arbitration may be shared between them, which is the property that makes the topology parallel rather than merely cheap. Every topology MUST keep every core reachable from the coordinator and MUST expose an identical `wau_highway_mesh` port interface (per-core `local_*`, per-line `hub_*`, per-line contract bus), so `wau_top`, the testbenches and the viewer stay single-sourced. Owned by `_render_highway_router` / `_render_highway_mesh` / `_render_fabric_binding` / `_render_defs` in [`verilog_emit.py`](src/python/waugen/verilog_emit.py) and `HighwaySpec` in [`config.py`](src/python/waugen/config.py); proven by [`tests/python/test_highway.py`](tests/python/test_highway.py) and [`tests/rtl/tb_wau_highway_lines.v`](tests/rtl/tb_wau_highway_lines.v).
...
host (MMIO write / vJTAG)
  ...
  → wau_coordinator          (up to MAX_IN_FLIGHT tagged transactions; same-flow inputs may overlap)
...
- **Key types:** `ProjectConfig` (root), `DeviceSpec` (preset, grid, widths/depths, `data_types`, `coordinator_mode`, `enable_runtime_auto_adapt`, `highway`), `HighwaySpec` (topology + contract-bus limits), `OperationSpec`, `FlowSpec` / `FlowStageSpec` / `FlowNodeSpec` (linear stages vs DAG nodes), `PlacementSpec` (`core` / `fallback_core` / `candidate_cores` / `fixed` / `directive`), `CoreCapabilitySpec`, `StationCacheSpec`, `StationProgramSpec` (the per-core fast-path table's `enabled`/`table_bits`), `CompilerSpec`, `SchedulerSpec`, `CoordinatorSpec`, `ProgramSpec`, `Coord`.
...
- **Tests:** [`tests/python/test_foundation_alignment.py`](tests/python/test_foundation_alignment.py), [`tests/python/test_3d_grid.py`](tests/python/test_3d_grid.py), [`tests/python/test_coordinator_config.py`](tests/python/test_coordinator_config.py), [`tests/python/test_station_program_config.py`](tests/python/test_station_program_config.py).
- **Common mistakes:** Adding a config key here without (a) a range check, (b) a matching emitter change or an explicit "Python-side only" note, and (c) a `README.md` config-model entry. `locality_bias` is the model for a Python-only knob; `max_in_flight` is the model for a knob that must reach a Verilog macro. `enable_runtime_auto_adapt` was previously the counter-example — validated here but never read by `verilog_emit.py` — until it was wired to `WAU_DEFAULT_AUTO_ADAPT` alongside the fast-path table.
...
- **Key functions and subparts:**
  ...
  - `build_fast_path_tables` — the per-core static fast-path dispatch table (see its contract in [Critical Implementation Contracts](#critical-implementation-contracts)): a read-only analysis pass over an already-compiled `CompiledProject`, never over `SchedulePlan`. Returns one `CoreFastPathTable` per core (`hops: dict[(flow_id, stage_index), FastPathHop]`, `overflowed_keys`). `_core_index_for` mirrors `scheduler.core_index`'s formula (duplicated, not imported, since `scheduler.py` imports from this module).
...
- **Tests:** [`tests/python/test_foundation_alignment.py`](tests/python/test_foundation_alignment.py), [`tests/python/test_advanced_scheduler.py`](tests/python/test_advanced_scheduler.py), [`tests/python/test_3d_grid.py`](tests/python/test_3d_grid.py), [`tests/python/test_station_program_table.py`](tests/python/test_station_program_table.py).
- **Common mistakes:** Skipping the capability filter when resolving candidates. `manual` routing with no explicit placement must fail loudly, not silently fall back to `waterfall`. Building `build_fast_path_tables` from `SchedulePlan` instead of `CompiledProject`. Also never let `_select_core` escape the compiled `primary_core`/`fallback_core`: extra candidates guide compilation, but no generated runtime packet can target them.
...
- **Key functions and subparts:**
  ...
  - `_build_runtime_nodes` — expands **all** programs × replicas × recurrence iterations into `_RuntimeNode`s with canonical `runtime_node_key`s (the determinism anchor), then adds ordering-only wave edges for `max_parallel_flows` instead of truncating instances.
  - `_select_program` — applies `scheduler.program_policy` (`weighted_fair`, `strict_priority`, `round_robin`) and per-program `load_balance`.
  - `_select_core` — executable primary/fallback choices only; earliest-free-cycle first, then `_locality_cost` (hop-distance weighting by `locality_bias`), then explicit deterministic keys. `_earliest_candidate_start` lets the ready-node list scheduler prefer work that can actually begin sooner.
...
- **Key functions and subparts (one renderer per emitted file):**
  ...
  - `_render_defs` — `wau_defs.vh`: the **single owner** of `WAU_GRID_X/Y/Z`, `WAU_CORE_COUNT`, `WAU_COORD_MAX_IN_FLIGHT`, `WAU_STATION_CACHE_*`, `WAU_STATION_PROGRAM_ENABLE`/`WAU_STATION_PROGRAM_TABLE_BITS`, `WAU_DEFAULT_AUTO_ADAPT`, and per-operation opcode macros.
  ...
  - `_render_core_station` — dispatch, latency control, configurable FIFO/LRU multi-entry cache, `cache_hit_count`/`cache_lookup_count`, and (new) the fast-path self-dispatch port (`self_*`/`peer_busy`) and `fast_path_lookup` function (rendered by `_core_fast_path_lookup_function`/`_fast_path_word_layout`, one packed-word `case ((flow_id, stage_id))` lookup per `CORE_INDEX`, following the same case-arm idiom as `_stage_case_entries` rather than a literal indexed ROM — see the fast-path contract above for why). `_fast_path_excluded_destinations` decides, per topology, which physical core indices `build_fast_path_tables` must never target.
  - `_render_neighbor_forward`, `_render_highway_router`, `_render_highway_mesh`, `_render_highway_contract` — the packet fabric, rendered per `device.highway.topology`. Under the default `lines` topology the router keeps `LOCAL`/`PREV`/`NEXT` and routes by asking whether `dst_core` lies further along **its own line** (`LINE_LAST` is an elaboration-time constant); anything else heads `PREV` and falls out of the line's west end into its hub, including the reserved all-ones `HUB_DST`. Under `chain` the same three ports plus `UP`/`DOWN` route by comparing `dst_core` against `CORE_INDEX`. Under `matrix` it computes `dst_x = dst_core % GRID_X`, `dst_y = (dst_core / GRID_X) % GRID_Y`, `dst_z = dst_core / (GRID_X*GRID_Y)` and routes **X first, then Y, then Z**. All three share one arbitration/observability body (`_HIGHWAY_ROUTER_BODY`) so they cannot drift.
  - `_render_fabric_binding` — the coordinator↔fabric wiring, which is what actually differs between topologies. Under `lines` the coordinator sits on one hub per line (dispatch demuxed by compile-time line bounds, results round-robined back) and every core uses its own local port on both planes; under `chain`/`matrix` the coordinator keeps sharing core 0's local port and the hub ports are inert. Both templates' `data_local_in_dst` is now table-driven (`core_result_dst_core`, per-core `wau_core_station` output) rather than a hardwired constant — it degenerates to the old hardwired hub/core-0 sentinel whenever a core's fast-path lookup misses, which is always true when the table is empty.
  ...
  - `_render_coordinator` (+ `_stage_case_entries`, `_flow_last_stage_entries`, `_flow_slot_entries`) — the tagged multi-issue transaction orchestrator and its `MAX_IN_FLIGHT` localparam. It admits by free-slot capacity, matches the packet-carried tag while accepting fast-path stage advancement, and reorders completed slots onto the host stream by acceptance order.
  - `_render_top` — the grid top level and the `obs_total_*` aggregation; also computes each topology's `COORD_DST_SENTINEL` and owns both internal payload layouts. `CTRL_TAG_LSB` and `DATA_TAG_LSB` place the 8-bit transaction tag at bit zero; the existing flow/stage/value fields and `DATA_IS_FAST_PATH_LSB`/`DATA_NEXT_*_LSB`/`DATA_B_REG_LSB` follow it. None of these internal fields alter the host/MMIO ABI.
  - `_render_host_mmio` — the fixed 32-bit register map; `enable_auto_adapt`'s reset value is now `` `WAU_DEFAULT_AUTO_ADAPT `` (config-driven) rather than the literal `1'b1`, still live-toggleable over MMIO CTRL bit 1 exactly as before.
  - `_render_de0_nano_wrapper` — board wrapper, emitted only under the `de0_nano` gate.
  - `_render_program_json` (+ `_render_station_program_report`) — the `wau_program.json` report, including the `station_program` observability section (mirrors exactly what `_core_fast_path_lookup_function` emitted, via the same `_fast_path_excluded_destinations` call).
...
- **Tests:** all of [`tests/rtl/`](tests/rtl) (via [`scripts/run_iverilog_tests.sh`](scripts/run_iverilog_tests.sh)), plus [`tests/python/test_coordinator_config.py`](tests/python/test_coordinator_config.py), [`tests/python/test_3d_grid.py`](tests/python/test_3d_grid.py), and `StationProgramJsonTests` in [`test_highway.py`](tests/python/test_highway.py) for macro/metadata/report assertions.
...
- [`wau_station_program_demo.json`](src/python/configs/wau_station_program_demo.json) — **contract:** the per-core fast-path table turned on (`compiler.station_program.enabled=true`), with flow 1's three stages explicitly pinned to distinct cores (`allow_adaptive: false` throughout, so placement stays fully deterministic) plus a third, fully disjoint flow for inter-flow concurrency. The exact fixture behind [`tb_wau_core_fast_path_overlap.v`](tests/rtl/tb_wau_core_fast_path_overlap.v) — do not change its flow/value shape without updating that testbench's hardcoded cycle counts and golden values in the same edit.
- [`wau_station_program_demo_disabled.json`](src/python/configs/wau_station_program_demo_disabled.json) — **contract:** byte-identical to `wau_station_program_demo.json` except `compiler.station_program.enabled=false`, so the two are compared like for like — the same pattern as `wau_matrix_highway_demo.json` above, just for this knob instead of highway topology. Backs [`tb_wau_station_program_degenerate.v`](tests/rtl/tb_wau_station_program_degenerate.v).
- [`wau_station_program_overflow_demo.json`](src/python/configs/wau_station_program_overflow_demo.json) — **contract:** a minimal 2x2 grid, `table_bits=1` (capacity 2), and three independent flows whose stage 0 all place on the same core — deliberately more distinct `(flow_id, stage_index)` pairs than the table can hold, so one always overflows. Backs [`tb_wau_station_program_overflow.v`](tests/rtl/tb_wau_station_program_overflow.v).
...
- **Key subparts:** `run_test <toplevel> <sources...>` — compiles with `iverilog -g2005-sv -I $RTL_DIR -s <name>` and runs `vvp`; `mesh_sources` / `top_sources` are the shared source sets; `run_suite` is the whole fabric suite. Runs, in order: `tb_wau_operation_alu`, `tb_wau_top_demo`, `tb_wau_coordinator_multiissue`, `tb_wau_highway_mesh`, `tb_wau_highway_contract`, `tb_wau_host_mmio`, then the topology-gated ones — `tb_wau_highway_lines` under `lines` RTL, `tb_wau_highway_mesh_3d` otherwise (vertical links exist only where one highway spans the grid), and `tb_wau_highway_chain`/`_3d` under `chain` RTL. It then **regenerates a further RTL tree per opt-in topology** from [`wau_chain_highway_demo.json`](src/python/configs/wau_chain_highway_demo.json) and [`wau_matrix_highway_demo.json`](src/python/configs/wau_matrix_highway_demo.json) and re-runs `run_suite` against each, so no topology ships unelaborated. A further loop (parallel arrays `STATION_PROGRAM_CONFIGS`/`STATION_PROGRAM_TESTBENCHES`, plain indexed bash arrays — **not** `declare -A`, to stay bash-3.2-compatible for a bare macOS/minimal-CI `env bash`) regenerates from each of the three `wau_station_program_*` configs and runs only that config's one dedicated testbench (not the whole `run_suite` — these configs' flows/values are purpose-built for one assertion each), so the fast-path table is never unelaborated either.
...
- [`tb_wau_highway_lines.v`](tests/rtl/tb_wau_highway_lines.v) — **the default topology's contract**: all-pairs reachability within a line, off-line traffic leaving through *that line's own* hub without leaking to another's, a hub reaching every core on its line, and — the point of the topology — that wedging one line's hub leaves the other line accepting and delivering in the same cycle. Meaningful only against `lines` RTL.
- [`tb_wau_highway_chain.v`](tests/rtl/tb_wau_highway_chain.v) — the chain on a multi-row grid: all-pairs reachability over one chain, the row-to-row wrap hop, and that a disabled contract bus is completely inert. Meaningful only against `chain` RTL.
- [`tb_wau_highway_chain_3d.v`](tests/rtl/tb_wau_highway_chain_3d.v) — the layered case: one chain **per layer**, joined only by the vertical links. Sweeps all 56 ordered pairs on a 2x2x2 grid, because a chain-per-layer arrangement is where dropping the planar links could silently partition the fabric.
...
- [`tb_wau_core_fast_path_overlap.v`](tests/rtl/tb_wau_core_fast_path_overlap.v) — the per-core fast-path table's payoff: two independent 3-stage flows (`wau_station_program_demo.json`) complete in fewer cycles (15 vs. the disabled twin's 17) with `≥2` cores concurrently busy, plus a third flow whose fast-path hop crosses highway lines and must still resolve correctly (safely absorbed by the coordinator, no hub-to-hub bridge — see the fast-path contract).
- [`tb_wau_station_program_degenerate.v`](tests/rtl/tb_wau_station_program_degenerate.v) — the same scenario with the table disabled (`wau_station_program_demo_disabled.json`) must reproduce the legacy 17-cycle timing exactly, byte-for-byte.
- [`tb_wau_station_program_overflow.v`](tests/rtl/tb_wau_station_program_overflow.v) — three flows competing for one core's 2-entry table (`wau_station_program_overflow_demo.json`, `table_bits=1`); the one that overflows must still resolve correctly via the legacy path.
...
Static layout model derived from `wau_program.json` + `wau_schedule.json`. Owns the seeded stress-stimulus generator (repeated same-flow packets are valid pipeline stress because transaction tags disambiguate them), the real-data MNIST stimulus (`derive_mnist_stimulus`: consecutive image bytes paired and centered to `[-128,127]` — this **MUST** stay identical to the DE0-Nano runner's `--mnist-images` path in [`run_cw_stress_benchmark.py`](demo/de0-nano/basic-example/host/programs/run_cw_stress_benchmark.py), or an animated run and a board run stop showing the same values), `HighwayInfo` (read back from the program report), and the route/segment reconstruction: `line_route`/`line_segments` for the default per-line highways, `chain_route`/`chain_segments` for `chain`, and `manhattan_route`/`matrix_segments` for `matrix`, dispatched by `WauModel.highway_route` / `highway_segments`. **All three MUST mirror `wau_highway_router`** — `manhattan_route` its X-then-Y-then-Z priority, `chain_route` its index compare, `line_route` its line-bounds compare (so an off-line route stops at the line's hub end rather than crossing into another line, which has no link to cross). If the emitter's routing changes, these change with it. Tested by [`tests/python/test_viewer_sim_prep.py`](tests/python/test_viewer_sim_prep.py).

#### [`wau_viewer/graph_view.py`](tools/wau-pipelines-viewer/wau_viewer/graph_view.py)

Zoomable `QGraphicsView` of the mesh. Owns the phased packet animation (dispatch → execute → result), the offset parallel lanes for concurrent packets, the in-scene concurrency HUD (busy cores, packets in flight, peak parallel ops, hops/stalls), and the **highway scheme** — `HighwayStub` per core, `HighwaySlotMarker` for the offered slot, and the contract HUD line. The scheme is driven entirely by the trace's `HWY`/`hwy_*` records, never inferred. A per-core fast-path hop (`CoreState.ddeliv_fastpath`, from `compiler.station_program`) renders as a distinct amber track (`COLOR_PACKET_FASTPATH`, reusing `COLOR_HWY_CALL`'s hue) labeled with the next operation it is about to run, rather than the plain blue "result over data mesh" track — a direct core→core handoff has no "return to the coordinator" leg to animate separately.

**All fabric geometry is orthogonal and gutter-routed** (`link_points`, `gutter_y`, `GUTTER_CHAIN`/`GUTTER_BUS`). A straight centre-to-centre line is only correct for physically adjacent cores; drawn that way, the chain's row-to-row hop becomes a diagonal across the grid and an ordered topology reads as chaotic. It is instead dropped into the row gutter and run back along it, and the contract bus is a branch per row (joined by a left spine) so each core's slot sits directly beneath it and no stub crosses another. `_route_points` walks each `MeshLink`'s own polyline via `points_from`, so animated packets stay on the wire as drawn — **never** re-interpolate core centres there, or packets will visibly take paths the fabric does not have. Playback is paced by a deterministic `advance(t)` clock so headless recordings replay the same frames as live playback.
...
[`__main__.py`](tools/wau-pipelines-viewer/wau_viewer/__main__.py) (entry point: prepare → simulate → launch Qt. `--headless` **always** short-circuits before any interactive `QApplication`/`app.exec()` and forces `QT_QPA_PLATFORM=offscreen` — previously it was silently ignored unless paired with `--record`, falling through to a real, interactive window and a blocking event loop; see [the pitfall](#--headless-silently-opened-a-real-window-without---record)), [`simulator.py`](tools/wau-pipelines-viewer/wau_viewer/simulator.py) (drives `iverilog`/`vvp`), [`tb_generator.py`](tools/wau-pipelines-viewer/wau_viewer/tb_generator.py) (generates the per-config tracing testbench; its `TB_DATA_*_LSB` chain mirrors `_render_top`'s data-plane payload layout exactly, starting with the 8-bit transaction tag and including the fast-path fields, and trace records carry `disp_tag`/`res_tag`/`ddeliv_tag`), [`trace_parser.py`](tools/wau-pipelines-viewer/wau_viewer/trace_parser.py) (parses the per-cycle trace and derives active-core count, peak/average busy cores, whole-fabric utilization, highway state, and stalls; covered by [`test_viewer_data_trace.py`](tests/python/test_viewer_data_trace.py)), [`main_window.py`](tools/wau-pipelines-viewer/wau_viewer/main_window.py) (transport controls, docks, recording — `run_headless_recording` takes the `--window-size` geometry so a recording can be shaped to the grid; its `win.show()`-then-`win.hide()` trick to get an off-screen-rendered widget is fragile across platforms, which is why `__main__.py` now forces the `offscreen` Qt platform plugin ahead of it rather than relying on `hide()` alone), [`timeline_view.py`](tools/wau-pipelines-viewer/wau_viewer/timeline_view.py) (Gantt timeline with playhead; `follow_cycle` keeps the playhead in view, without which a trace longer than the makespan plays against a blank strip), [`stats_panel.py`](tools/wau-pipelines-viewer/wau_viewer/stats_panel.py) (bottleneck readout). The headline fixture is [`wau_4x4_pipeline_demo.json`](tools/wau-pipelines-viewer/examples/wau_4x4_pipeline_demo.json): four pinned row pipelines, 32 scheduled instances, a 16-transaction runtime window, and a directed test requiring all 16 cores to dispatch.
...
### Tagged multi-issue coordinator — Shipped

- **Behavior:** `coordinator.max_in_flight` tagged transactions execute concurrently across the mesh (one accumulator context per slot). The slot id rides every internal dispatch/result/fast-path packet, so repeated inputs of one flow id can fill successive pipeline stages alongside independent flows. `1` reproduces the legacy serial coordinator exactly.
...
- **Constraints:** `[1,16]`; the tag is internal and does not alter host/MMIO ports; unknown flow ids remain accepted-but-dropped.
- **Tests and gaps:** [`tb_wau_coordinator_multiissue.v`](tests/rtl/tb_wau_coordinator_multiissue.v), [`tb_wau_top_demo.v`](tests/rtl/tb_wau_top_demo.v) (serial equivalence), [`test_coordinator_config.py`](tests/python/test_coordinator_config.py), and the 4x4 all-core viewer simulation in [`test_viewer_data_trace.py`](tests/python/test_viewer_data_trace.py). Intra-flow DAG fan-out is still not a runtime feature; linearized stages gain throughput by overlapping different tagged inputs.

### Per-core fast-path table — Shipped

- **Behavior:** `compiler.station_program.enabled` (default `false`) lets a core hand a completed non-final stage's result **directly to the next stage's core**, over the data plane's local port, instead of routing back through `wau_coordinator` — collapsing what was a coordinator round-trip per interior stage into a single hop, and lifting the "one dispatch per cycle, system-wide" ceiling for those hops. `table_bits` (default `5`, `[1,8]`) bounds each core's table to `2**table_bits` entries. This is what closes the gap the README's pre-existing demo recordings showed: previously every stage of every flow, without exception, waited a full coordinator round-trip before the next could start, however many cores sat idle in the meantime.
- **Flow and owners:** [`config.py`](src/python/waugen/config.py) (`StationProgramSpec`) → [`compiler.py`](src/python/waugen/compiler.py) (`build_fast_path_tables`, `FastPathHop`, `CoreFastPathTable` — the "concurrence ID" is `FastPathHop.concurrence_id`, a deterministic per-core ordinal that bounds/reports capacity; the RTL lookup itself is keyed by `(flow_id, stage_index)`, not by this id) → [`verilog_emit.py`](src/python/waugen/verilog_emit.py) (`_core_fast_path_lookup_function`, `_fast_path_word_layout`, `_fast_path_excluded_destinations`, plus the `wau_core_station`/`wau_core`/`wau_top`/`_render_fabric_binding`/`_render_coordinator` changes described in the contract above) → `wau_core_station.v`'s `fast_path_lookup` function, the widened data-plane payload, and `wau_program.json`'s `station_program` section (`_render_station_program_report`).
- **Constraints:** See the **Per-core fast-path table contract** above (capacity, exclusion rules, the coordinator relaxation, cross-line absorption). Additive and always-safe: an empty/disabled table degenerates the generated RTL to exactly today's behavior (`in_ready` reduces to `state == ST_IDLE`, `data_local_in_dst` still resolves to the hub/core-0 sentinel). `device.enable_runtime_auto_adapt` now actually reaches RTL (previously a dead config field) and defaults to `false`.
- **Tests and gaps:** [`tb_wau_core_fast_path_overlap.v`](tests/rtl/tb_wau_core_fast_path_overlap.v) (genuine speedup on [`wau_station_program_demo.json`](src/python/configs/wau_station_program_demo.json): 15 cycles vs. 17 disabled, for two independent 3-stage flows; also covers cross-line absorption via that config's `max_then_div` flow), [`tb_wau_station_program_degenerate.v`](tests/rtl/tb_wau_station_program_degenerate.v) (disabled twin [`wau_station_program_demo_disabled.json`](src/python/configs/wau_station_program_demo_disabled.json), byte-identical 17-cycle timing), [`tb_wau_station_program_overflow.v`](tests/rtl/tb_wau_station_program_overflow.v) ([`wau_station_program_overflow_demo.json`](src/python/configs/wau_station_program_overflow_demo.json), `table_bits=1`, one of three competing flows overflows and still resolves correctly), [`test_station_program_table.py`](tests/python/test_station_program_table.py), [`test_station_program_config.py`](tests/python/test_station_program_config.py), `StationProgramJsonTests` in [`test_highway.py`](tests/python/test_highway.py). Gap: `build_fast_path_tables` does not yet avoid allocating capacity to a hop whose destination is on a different highway line under `lines` (harmless — see the contract above — but wasteful); not yet swept by `fit-config`/`arch_search.py`; no `@wau` pragma exposes it to `.cw` kernels; true multi-consumer DAG fan-out (one result feeding two independent downstream branches) is out of scope, not just deferred — see [ROADMAP.md](ROADMAP.md).
...
- **Behavior:** `device.highway.topology` selects how the highway is laid over the grid. `lines` (**the default**) gives **one independent highway per line of cores** — `grid.y * grid.z` of them, three router ports each, every one reaching the coordinator through its own hub — so rows carry traffic in parallel rather than sharing a wire. `chain` gives each layer a single 1-D highway walked in core-index order (5 ports, layers joined vertically): fewest links, but everything serialises on it. `matrix` restores the full N/S/E/W(/U/D) mesh with X-then-Y-then-Z routing for traffic that needs the cross-section.
- **Flow and owners:** [`config.py`](src/python/waugen/config.py) (`HighwaySpec`, `line_count`/`line_size`) → [`verilog_emit.py`](src/python/waugen/verilog_emit.py) (`_highway_dirs`, `_render_highway_router`, `_render_highway_mesh`, `_render_fabric_binding`, `_render_defs`) → `wau_highway_router.v` / `wau_highway_mesh.v` / `wau_top.v` / `WAU_HIGHWAY_*`; mirrored in the viewer by `model.line_route` / `line_segments`.
- **Constraints:** The `lines` and `chain` routers must stay free of `% GRID_X` and `/ GRID_X` — that is what removes the per-port `LPM_DIVIDE` and with it the [non-power-of-two LE blow-up](#non-power-of-two-grid-blows-the-le-budget) *for any grid shape*. Per-line hubs cost no reachability **only because, in the dynamic dispatch model, cores never address each other** (all coordinator-mediated traffic is coordinator↔core). **Core-to-core traffic has since been introduced** by the per-core fast-path table (`compiler.station_program`, see its contract above) — but only ever *within* a line: a hop's destination is `wau_core_station`'s own local port, reached by the same route any off-line coordinator-bound traffic already used (walk west, leave through this line's hub). There is still no hub-to-hub bridge, so a fast-path hop whose destination happens to be on a *different* line is safely absorbed by the coordinator instead (see the fast-path contract's "no speedup, still correct" note) rather than crossing directly — `lines` genuinely still has no real inter-line path, only a documented, harmless degradation for the one case that would otherwise need one. The trade is highway cross-section: one path between any two cores on a line. Neither opt-in topology is ever the default, and CI must keep elaborating both (see [`wau_chain_highway_demo.json`](src/python/configs/wau_chain_highway_demo.json), [`wau_matrix_highway_demo.json`](src/python/configs/wau_matrix_highway_demo.json)) or those paths ship unexercised.
- **Tests and gaps:** [`test_highway.py`](tests/python/test_highway.py), [`tb_wau_highway_lines.v`](tests/rtl/tb_wau_highway_lines.v), plus the whole fabric suite run three times by [`run_iverilog_tests.sh`](scripts/run_iverilog_tests.sh). Gap: the schedule's `dependency_edges_v1` transfer-hop metric and `_coord_distance` still model Manhattan distance, which does not match either one-dimensional topology (see [Known Gaps](#known-gaps)); no board calibration of the area saving, and the per-line parallelism is proven structurally in simulation but not yet measured as throughput.
...
- **Constraints:** `arch-search` output is byte-frozen and uses its private v1 compiler/scheduler compatibility path; `fit-config` is additive, in-memory only, and uses current placement/scheduling. Area/BRAM/DSP figures come from the versioned estimate models, **not** from a synthesis tool.
- **Tests and gaps:** [`test_arch_search.py`](tests/python/test_arch_search.py), including the full report hash. Gaps: estimates are not calibrated against real Quartus/Vivado results or board-measured scores; the automatic `max_in_flight` sweep uses declared flow count and needs an explicit override for a repeated one-flow stream (tracked in `ROADMAP.md`).
...
- **Status:** Deliberate limitation of the **`matrix`** router arithmetic; documented in [`README.md`](README.md) and [`benchmarks/de0_nano_cw_stress_benchmark.txt`](benchmarks/de0_nano_cw_stress_benchmark.txt). The default `lines` topology (and `chain`) has no `% GRID_X` / `/ GRID_X` at all, so it does not have this failure mode for any grid shape — but the board images recorded in `benchmarks/de0_nano_*` predate it and were built on the `matrix` fabric. Do not retro-apply the saving to those numbers.
...
- **Cause and invariant:** `model.highway_route` reconstructs routes in Python; the authority is `_render_highway_router`. There are now three orders to keep mirrored — the line-bounds compare of the default `lines`, the index compare of `chain`, and the X-then-Y-then-Z dimension order of `matrix` — and the viewer must pick between them from `wau_program.json`'s `device.highway.topology`, not from a hardcoded assumption. Under `lines` a route must also *stop* at its line's hub end rather than continuing into another line, which has no link to continue over.
...
- **Safe pattern / regression check:** Any routing-order change updates both in one edit. `ManhattanRouteTests` asserts X-first ordering against neighbor steps, and `LineHighwayRouteTests` asserts line-local walking, that a row boundary is *not* a hop, and that every reconstructed hop is a drawn highway link **for all three topologies** — both in [`test_viewer_sim_prep.py`](tests/python/test_viewer_sim_prep.py).
- **Status:** Fixed failure mode; live regression risk.

### `--headless` silently opened a real window without `--record`

- **Symptom / wrong assumption:** Running `python3 -m wau_viewer ... --headless` without also passing `--record` (e.g. to only use `--dump-trace`) opens a real, interactive window on the caller's display and blocks in `app.exec()` — the opposite of what `--headless` promises. Observed as a window that "does nothing" and never returns control, on a machine with a real display attached (not a true CI/no-display box).
- **Cause and invariant:** `__main__.py`'s dispatch only special-cased `if args.record and args.headless:`; any other combination — including `--headless` alone — fell straight through to the interactive branch (`QApplication(sys.argv)`, `win.show()`, `app.exec()`). Separately, even the correct `--record --headless` path only made its window invisible via `run_headless_recording`'s `win.show()`-then-`win.hide()` trick, which is not guaranteed to actually hide the native window on every platform/Qt version.
- **Risk area:** [`__main__.py`](tools/wau-pipelines-viewer/wau_viewer/__main__.py), [`main_window.py`](tools/wau-pipelines-viewer/wau_viewer/main_window.py) (`run_headless_recording`).
- **Safe pattern / regression check:** `--headless` must **always** short-circuit before any `QApplication`/`app.exec()` is reached — return immediately if `--record` was not also given (nothing further needs Qt), and force `QT_QPA_PLATFORM=offscreen` before importing `PySide6.QtWidgets` whenever it *was* given, so recording never depends on `hide()` succeeding. Manually verified: `--headless` alone now returns in ~1s with no window; `--headless --record` completes in-process with no window and a valid GIF, confirmed via `ps aux` showing no lingering process either way.
- **Status:** Fixed.
...
| `wau_core` / `wau_core_station` / `wau_operation_alu` | `_render_core*`, `_render_operation_alu` | `CORE_INDEX`-gated ALU elaboration; cache counters; fast-path self-dispatch port group (`self_*`/`peer_busy` in, `result_dst_core`/`result_is_fast_path`/`result_next_*`/`result_b_reg` out) and the `fast_path_lookup` function |
...
**File formats produced:** `wau_program.json` (`_render_program_json`, including the `station_program` observability section from `_render_station_program_report`), `wau_schedule.json` (`SchedulePlan.to_json`), `wau_schedule.hex` (`encode_instruction_word`, 64-bit words).
...
```bash
python3 -m wau_viewer --config examples/wau_4x4_pipeline_demo.json --stress 32 --record examples/wau_4x4_pipeline_demo.gif --framerate 12 --frames-per-cycle 2 --gif-width 1800 --window-size 2600x2100 --headless
```
...
| Per-core fast-path table: concurrence-ID assignment, capacity/overflow, destination exclusion | [`test_station_program_table.py`](tests/python/test_station_program_table.py) |
| `compiler.station_program` schema, `enable_runtime_auto_adapt` default | [`test_station_program_config.py`](tests/python/test_station_program_config.py) |
| Fast-path speedup + cross-line safety, disabled byte-identity, capacity overflow | [`tb_wau_core_fast_path_overlap.v`](tests/rtl/tb_wau_core_fast_path_overlap.v) + [`tb_wau_station_program_degenerate.v`](tests/rtl/tb_wau_station_program_degenerate.v) + [`tb_wau_station_program_overflow.v`](tests/rtl/tb_wau_station_program_overflow.v) |
...
| `wau_program.json`'s `station_program` observability section matches the emitted RTL | `StationProgramJsonTests` in [`test_highway.py`](tests/python/test_highway.py) |
| Per-line highway independence, hub egress/ingress, no cross-line leakage | [`tb_wau_highway_lines.v`](tests/rtl/tb_wau_highway_lines.v) |
| Chain routing incl. the row-to-row wrap hop, inert disabled bus | [`tb_wau_highway_chain.v`](tests/rtl/tb_wau_highway_chain.v) |
| Layered chain highway: chain per layer, all-pairs cross-layer reachability | [`tb_wau_highway_chain_3d.v`](tests/rtl/tb_wau_highway_chain_3d.v) |
...
- **Canonical vs derived.** Canonical: [`src/python/waugen/`](src/python/waugen), [`src/python/configs/*_demo.json`](src/python/configs) (including `wau_station_program_demo_disabled.json`, which does not match that glob but is exactly as canonical/hand-written as its `_demo.json`-named twin) and the two contract configs, [`CWs/`](CWs), [`tests/`](tests), [`scripts/`](scripts), [`demo/`](demo/de0-nano/basic-example) RTL/host sources. Derived and regenerable: everything in `src/verilog/generated/`, `src/python/configs/*compiled*.json`, `benchmarks/*.json` sidecars. Untracked runtime: `.build/` (all build/simulation scratch) and `datasets/`.
...
- Per-core fast-path table (`compiler.station_program`): a core hands a completed non-final stage's result directly to the next stage's core instead of round-tripping `wau_coordinator`, cutting interior-stage latency (measured 15 vs. 17 cycles on the two-flow demo) while staying byte-identical when disabled (the default). `device.enable_runtime_auto_adapt` now actually reaches RTL and defaults to off.
...
- **Transfer-hop metric assumes a mesh.** `_coord_distance` (compiler) and the `dependency_edges_v1` metric (scheduler) both model Manhattan distance, which is the true hop count only on a `matrix` highway; on the default per-line highway the real hop count is the offset within the line plus the hub, and on `chain` it is the index delta, so the reported estimate matches neither. Not corrected because `arch-search` output is byte-frozen and ranks on that metric. Treat `estimated_transfer_hops_*` as a mesh-relative estimate, and use the RTL `obs_total_hop_count` for the real figure.
- **Highway topology not board-calibrated.** The per-line topology's area/LE saving is argued from port and link counts and verified in `iverilog`; it has **not** been through Quartus. Its throughput advantage over `chain` is likewise proven structurally (lines do not share back-pressure) but has not been measured as a benchmark delta. Do not report either as measured.
